...
class LLMStream(ABC):
    ...
    async def _metrics_monitor_task(self, event_aiter: AsyncIterable[ChatChunk]) -> None:
        ...
        rtf = duration / usage.total_tokens if usage else 0.0
        logger.info(
        f"LLM start={start_time:.4f} end={end_time:.4f} "
        f"duration={duration:.4f}s tokens={usage.total_tokens} RTF={rtf:.3f}"
        )
